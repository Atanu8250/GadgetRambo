...
import { Button, Divider, Flex, Heading, Image, Show, Text } from "@chakra-ui/react";
import { doc, getDoc } from "firebase/firestore";
import React from "react";
import { AiFillFacebook, AiFillLinkedin, AiFillMail, AiFillRedditCircle, AiFillTwitterCircle, AiOutlineComment } from "react-icons/ai";
...
const Blog = ({ newsData }: newsBlogId) => {
  ...
  return (
    <>
      <Flex px={10}>
        {/* Center Side */}
        <Flex p={5} flex={8} justifyContent={"center"} direction={"column"} alignItems={"flex-start"}>
          <Flex mb={5}>
            <Heading size={"lg"}>{newsData.title}</Heading>
          </Flex>
          <Flex py={6} gap={2} wrap={"wrap"}>
            <Button fontSize={"12px"} colorScheme={"facebook"} h={7} borderRadius={0}>
              <AiFillFacebook style={{ marginRight: "5px" }} />
              Share on Facebook
            </Button>
            <Button fontSize={"12px"} colorScheme={"twitter"} h={7} borderRadius={0}>
              <AiFillTwitterCircle style={{ marginRight: "5px" }} /> Tweet
            </Button>
            <Button fontSize={"12px"} colorScheme={"yellow"} h={7} borderRadius={0}>
              <BsSnapchat style={{ marginRight: "5px" }} />
              Snapchat
            </Button>
            <Button fontSize={"12px"} colorScheme={"linkedin"} h={7} borderRadius={0}>
              <AiFillLinkedin style={{ marginRight: "5px" }} /> Share
            </Button>
            <Button fontSize={"12px"} colorScheme={"blue"} h={7} borderRadius={0}>
              <AiFillRedditCircle style={{ marginRight: "5px" }} /> Reddit
            </Button>
            <Button fontSize={"12px"} colorScheme={"gray"} h={7} borderRadius={0}>
              <AiFillMail style={{ marginRight: "5px" }} /> Email
            </Button>
            <Button fontSize={"12px"} colorScheme={"purple"} h={7} borderRadius={0}>
              <AiOutlineComment style={{ marginRight: "5px" }} /> Comment
            </Button>
            <Button fontSize={"12px"} colorScheme={"green"} h={7} borderRadius={0}>
